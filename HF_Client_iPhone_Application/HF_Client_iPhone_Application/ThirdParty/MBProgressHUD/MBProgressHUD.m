...
+ (MBProgressHUD *)jihaiProgressHUDWithSuperview:(UIView *)superview andDelegate:(id<MBProgressHUDDelegate>)delegate
{
    MBProgressHUD *progressHUD = [[MBProgressHUD alloc] initWithView: superview];
    progressHUD.opacity = 0;
    progressHUD.labelFont = [UIFont boldSystemFontOfSize: 15];
    progressHUD.mode = MBProgressHUDModeJIH;
...
- (void)setWillHideLabel:(BOOL)willHideLabel
{
    _willHideLabel = willHideLabel;
    label.hidden = willHideLabel;
}
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		[indicator removeFromSuperview];
		self.indicator = nil;
    } else if (mode == MBProgressHUDModeJIH) {
        [indicator removeFromSuperview];
        self.indicator = [[[NSBundle mainBundle] loadNibNamed: @"JIHProgressHUD" owner: self options: nil] lastObject];
